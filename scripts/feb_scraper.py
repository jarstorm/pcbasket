...
def main():
    ...
    with open("feb_primerafeb_2025_26_raw.json", "w", encoding="utf-8") as f:
        json.dump({"season": "2025/2026", "competition": "Primera FEB", "teams": result}, f, ensure_ascii=False, indent=2)
    total_players = sum(len(t["roster"]) for t in result)
    with_stats = sum(1 for t in result for p in t["roster"] if p["stats"])
    print(f"Saved feb_primerafeb_2025_26_raw.json — {total_players} players, {with_stats} with stats")
